Shader "Jettelly/CristalCg"
{
    Properties
    {
        ...
        _InnerFresnelFactor ("Inner Fresnel Factor %", Range(0, 1)) = 0.9
        ...
        _AnisoFactor ("Anisotropic Factor", Range(0, 1)) = 1
    ...
    {      
        Pass
        {
            ...
            {    
                ...
                float3 worldNormal = normalize(mul(unity_ObjectToWorld, float4(i.normal, 0))).xyz; 
                ...
                for(y = -1; y <= 1; y++)
                {
                    for(x = -1; x <= 1; x++)
                    {
                        ...
                        warp *= (sin(noise * ((_WorldSpaceCameraPos.x - (i.worldPos.x * 5) + noise) * 20)));
                        warp *= (cos(noise * ((_WorldSpaceCameraPos.y - (i.worldPos.y * 5) + noise) * 20)));          
                        warp *= (sin(noise * ((_WorldSpaceCameraPos.z - (i.worldPos.z * 5) + noise) * 20)));
                ...
                float fresnel = 0;
                FresnelEffect_float(worldNormal, viewDir, _InnerFresnelPower, fresnel);  
                fresnel *= _InnerFresnelFactor;
                fresnel = 1 - fresnel;
                
                float3 reflectionColor = 0;
                float3 worldReflection = reflect(-viewDir, worldNormal);
        ...
        Pass
        {
            ...
            void AnisotropicSpecular_float(float nU, float nV, float3 tangentDir, float3 normalDir, float3 lightDir, float3 viewDir, float reflectionFactor, float anisoFactor, out float3 Out)
            {
                float pi = 3.141592;
            ...
            {    
                ...
                SpecularPhong_float(worldNormal, lightDir, viewDir, float3(2, 2, 2), _SpecularFactor, attenuation, _SpecularPower, specularColor);

                float fresnel = 0;
                FresnelEffect_float(worldNormal, viewDir, _FresnelPower, fresnel);  
                fresnel *= _FresnelFactor;
                
                float3 specularAnisoColor = 0;
                float4 tangentMap = tex2D(_TangentMap, i.uv);
                AnisotropicSpecular_float(_AnisoU, _AnisoV, UnpackNormal(tangentMap), worldNormal, lightDir, viewDir, _ReflectionFactor, _AnisoFactor,  specularAnisoColor);                
