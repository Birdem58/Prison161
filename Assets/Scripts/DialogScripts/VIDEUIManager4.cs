...
public class VIDEUIManager4 : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        //Load all dialogues to memory so that we dont spend time doing so later
    ...
    public void EndCurrentDialogue()
    {
        if (VD.isActive)
        {
            EndDialogue(VD.nodeData); // Call EndDialogue to handle everything properly
        }
    }
